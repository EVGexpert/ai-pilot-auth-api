 ...
 */
export async function createActionRequest({ userId, siteId, sessionId, action }) {
  const key = action.idempotency_key || generateActionKey(action)

  // Проверяем, есть ли уже такое действие
  const existing = await queryOne('SELECT * FROM action_requests WHERE idempotency_key = ?', [key])
  if (existing) return existing

  const id = uid(); const t = now()
  await run(`INSERT INTO action_requests (id, user_id, site_id, session_id, idempotency_key, action_type, action_json, status, result_json, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, 'pending', NULL, ?, ?)`,
    [id, userId, siteId || null, sessionId || null, key, action.type, JSON.stringify(action), t, t])
  return await queryOne('SELECT * FROM action_requests WHERE id = ?', [id])
}

/**
 * Получить запрос действия по idempotency ключу.
 */
export async function findActionByKey(key) {
  return await queryOne('SELECT * FROM action_requests WHERE idempotency_key = ?', [key])
}

/**
 * Обновить статус действия.
 */
export async function updateActionStatus(id, status, result = null) {
  ...
  params.push(id)
  await run(`UPDATE action_requests SET ${sets.join(', ')} WHERE id = ?`, params)
}

/**
 * Получить все действия для сессии.
 */
export async function getActionsBySession(sessionId, includeDone = false) {
  const statusFilter = includeDone ? '' : "AND status = 'pending'"
  return await queryAll(`SELECT * FROM action_requests WHERE session_id = ? ${statusFilter} ORDER BY created_at ASC`, [sessionId])
}
